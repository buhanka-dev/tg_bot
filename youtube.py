    ...
    if url.find('youtube') != -1 or url.find('youtu.be') != -1: # ссылка на ютуб?
        if url.find('&list=') != -1 and url.find('index=') == -1: # это плейлист?
            ...
            for video in playlist.videos:
                ...
                paths.append(f'cache/youtube/{path}.mp4')
            ...
            print('v')
            video = YouTube(url, 'WEB', use_oauth=True, allow_oauth_cache=True)
            path = re.sub("[^A-Za-z0-9 ]", "", video.title)
            ys = video.streams.get_highest_resolution()

            # запись файла в папку
            ys.download(output_path=f'cache/youtube/', timeout=120, filename=f'{path}.mp4')

            return [f'cache/youtube/{path}.mp4']
...
if __name__ == '__main__':
    download('https://www.youtube.com/watch?v=ax294Sq5phc')