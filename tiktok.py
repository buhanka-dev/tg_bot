...
def download_tiktok(link, filename):
    ...
    if soup.find_all('a')[0].text == 'Without watermark': # это видео?
        print('video')
        ...
        for u in soup.find_all('a'):
            if u.text == 'Download this slide':
                response = requests.get(u['href'], stream=True, headers=headers)
                path = f'cache/tiktok/{filename}_{random.random()}_{random.random()}.png'
                paths.append(path)
